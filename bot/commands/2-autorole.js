...
module.exports = {
  name: 'autorole',
  usage: 'autorole {mode} (roles...)',
  aliases: ['ar'],
  description: '自動付与ロールを追加/削除します(mode: `add, remove, reset`)',
    ...
    try {
      if (mode === 'add') {
        for (const item of result) {
          if (!settings.role.autos.includes(item)) {
            settings.role.autos.push(item);
          }
        }
      } else if (mode === 'remove') {
        if (settings.role.autos) {
          settings.role.autos = settings.role.autos.filter(item => !result.includes(item));
        }
      }
